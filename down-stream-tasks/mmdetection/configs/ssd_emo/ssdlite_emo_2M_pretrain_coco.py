...
model = dict(
    ...
    backbone=dict(
        ...
        pretrained='../../resources/EMO_2M/net.pth',
        frozen_stages=-1, norm_eval=True),
    neck=dict(
        type='SSDNeck',
        in_channels=(120, 200),
        out_channels=(120, 200, 512, 256, 256, 128),
    ...
    bbox_head=dict(
        type='SSDHead',
        in_channels=(120, 200, 512, 256, 256, 128),
        num_classes=80,
        use_depthwise=True,
        norm_cfg=dict(type='BN', eps=0.001, momentum=0.03),
        act_cfg=dict(type='ReLU6'),
...
bs_ratio = 1
data = dict(
    samples_per_gpu=24 * bs_ratio,
    workers_per_gpu=8 * min(bs_ratio, 2),
    train=dict(
        _delete_=True,
...
optimizer = dict(type='AdamW', lr=0.0002 * 6 * bs_ratio, betas=(0.9, 0.999), weight_decay=0.05,
